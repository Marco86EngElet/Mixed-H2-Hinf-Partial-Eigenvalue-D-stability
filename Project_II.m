clc, close all, clear all

format SHORT

%% 1) Inputs

    % 1.1) Number of spaces-states
    n_x=12;
    
    % 1.2) Number of outputs without disturb influence
    n_y=1;
    
    % 1.3) Quantidade de saidas com influencia direta de disturbios
    n_z=1;

    % 1.4) Number of disturbs
    n_d=1;    

    % 1.5) Number of inputs control 
    n_u=2;

    % 1.6) Number of allocable poles by feedback
    n_ola=n_u;
 
%% 2) Enter the number of real poles and complex conjugate pole pairs 

    %2.1) Number of real poles
    n_r=0;
    
    %2.2) Number of complex conjugate pole pairs 
    n_pcc=6;

%% 3) Parameters for D-stability LMIs
    
    alpha_v=0.8931;
    beta_v=4.4000;
    e_P=[];
    q_d=0;
    r_d=2.8074;
    w_H=[];
    theta_s=0.8405;
    
%% 4) Make Partial Fractions Functions

    %4.1) Damp ratio
    
    zeta_c(1,1) = 0.9^(1/30);  % Fraction 1
    zeta_c(2,1) = 0.9^(2/30); % Fraction 2
    zeta_c(3,1) = 0.9^(3/30); % Fraction 3
    zeta_c(4,1) = 0.9^(4/30);  % Fraction 4
    zeta_c(5,1) = 0.9^(5/30);  % Fraction 5
    
    zeta_c(6,1)=0.2; %Fraction 6
    
    %4.2) Natural frequency
    
    wn_c(1,1) = exp(-2+1/30);  % Fraction 1
    wn_c(2,1) = exp(-2+2/30); % Fraction 2
    wn_c(3,1) = exp(-2+3/30); % Fraction 3
    wn_c(4,1) = exp(-2+4/30);  % Fraction 4
    wn_c(5,1) = exp(-2+5/30);  % Fraction 5

    wn_c(6,1) = 0.1;  % Fraction 6

    %4.3) Gain
    
    KYc=0.5*ones(6,1); %"Gody(s)" 
    KYc(6,1)=10;
    
    KZc=0.5*ones(6,1); %"Godz(s)"
    KZc(6,1)=10; 

    %4.4) Make transfer functions "Gody(s)" e "Godz(s)" model 

        %4.4.1) Initialize "Gody(s)", "Godz(s)" and "GNA(s)"
    
        Godys=tf(0,1); % Gody(s)=0;
        Godzs=tf(1,1); % Godz(s)=0.01;
        GNAs=tf(0,1);%GNA(s)=0;
        
        %4.4.2) 'For' to sum partial fractions functions
        for c=1:6 
            num = wn_c(c,1)^2; % Numerator 
            den = [1, 2*zeta_c(c,1)*wn_c(c,1), wn_c(c,1)^2]; % Denominator  
            G_c = tf(num, den); % Partial Fraction Function
            Godys=Godys+KYc(c,1)*G_c; % Add partial fraction in "Gody(s)"
            Godzs=Godzs+KZc(c,1)*G_c; % Add partial fraction in "Godz(s)"
            if c<=5
                GNAs=GNAs+KYc(c,1)*G_c;
            else
                GAs=KYc(c,1)*G_c;
            end
        end

%% 5) Canonical Representation of Open-Loop System Transfer Functions
    
    %5.1) Gody(s) 
    
    Canon_Godys = ss(Godys);
    
    %5.2) Godz(s)
    
    Canon_Godzs = ss(Godzs);
    
    %5.3) Extract Matrices
    
    A_x=Canon_Godys.A;
    B_d=Canon_Godys.B;
    C_y=Canon_Godys.C;
    E_y=Canon_Godys.D;
    C_z=Canon_Godzs.C;
    E_z=Canon_Godzs.D; 
    
%% 6) Compute Norms and Poles of Open-Loop System
    
    Compute_Norms_and_Poles_Open_Loop
    
%% 7) Compute Natural Frequency and Damp Ratio

    [wn_God,zeta_God,p_God]=damp(Canon_Godys);
    
%% 8) Gody(s) Response of unit impulse disturb

    impulse_simulation_time=0:0.01:14;
    
    Compute_Impulse_Simulation_Data
    
%% 9) Input Control Matrices: 'B_u', 'D_y', 'D_z'.
    
    %9.1) B_u
    
    B_u=zeros(n_x,n_u);
    B_u(2,1)=1;
    B_u(3,1)=1;
    B_u(2,2)=-1;
    B_u(3,2)=-1;
    
    %9.2) D_y e D_z
    
    D_y=0.001*ones(n_y,n_u);
    D_z=D_y;
    
%% 10) Configure SDP optimization

    %10.1) Base yalmip settings for SDP 
    
    SDP_settings =...
         sdpsettings('verbose',1,'solver','lmilab','debug',1);

    %10.2) Cost Function Weights 
    
    c_H2=2;
    c_Hinf=1;

%% 11) Compute number of uncontrollabel systems states

    Compute_number_of_unallocatable_states

%% 12)  Make decision variables for SDP-LMI optimization

X  = sdpvar(n_x,n_x,'symmetric');
W  = sdpvar(n_u,n_x,'full');
gamma = sdpvar(1,1,'symmetric'); 
Z = sdpvar(n_y,n_y,'symmetric');
rho = sdpvar(1,1,'symmetric');   
        
%% 13)  Make LMIs for optimal mixed H2-Hinf control with D-stability

LMIs_mixed_control_D_stable

%% 14) Compute Theorem 2

tic %Start time counter
optimize(set_LMIs_classic,...
            c_H2*rho+c_Hinf*gamma,...
            SDP_settings);
classic_optimization_time=toc; %End time counter

%% 15) Extract optimal solution by Theorem 2

optimal_W = value(W);
optimal_X = value(X);

%% 16) Compute Feedback matrix by Theorem 2

Kpf_classic=optimal_W/optimal_X;

%% 17) State Space for Closed Loop System by Theorem 2

    % 17.1) State Space Gcdy(s) 
    
    Gcdys_classic=...
        ss(A_x+B_u*Kpf_classic,B_d,C_y+D_y*Kpf_classic,E_y);

    % 17.2) State Space Gcdz(s)  
    
    Gcdzs_classic=...
        ss(A_x+B_u*Kpf_classic,B_d,C_z+D_z*Kpf_classic,E_z);
    
%% 18) Compute norms H_2 e H_inf of Closed Loop System by Theorem 2 

    %18.1) Norms H_2 of Gcdy(s)
    
    H2_norm_Gcdys_classic=norm(Gcdys_classic,2);
    
    %18.2) Norms H_inf of Gcdz(s)
    
    Hinf_norm_Gcdzs_classic=norm(Gcdzs_classic,'inf');
    
%% 19) Poles of Closed Loop System by Theorem 2

    Poles_Gcd_classic=cplxpair(pole(Gcdys_classic));

%% 20) Response Gcdy(s) by unit impulse disturb

    Compute_yt_ut_impulse_response_classic_method
    
%% 21) Compute Theorem 2

    %21.1) Initialize feedback matrix
    
    Kpf_partial=zeros(n_u,n_x);

    %21.2) Input Q_p
    
    Q_p = [1 1;1i -1i];

    %21.3) Compute Eigenvalues and Eigenvectors
        
    [Right_Eigenvectors, Eigenvalues, Left_Eigenvectors] = ...
        eig(A_x + B_u * Kpf_partial);
        
    %21.4) Find allocable poles
    [row_poles,col_poles]=find(...
        real(Eigenvalues)<-0.019 & real(Eigenvalues)>-0.021)
    
    %21.5) Compute Matrices
    Lambda_p = Eigenvalues(row_poles, col_poles);
    L_p = Left_Eigenvectors(1:end, col_poles);    
        
    %21.6)  Compute mixed D-stable optimal control with partial allocation
    LMIs_mixed_control_partial_D_stable
    
        %21.7) Start optimization time counter
        tic
    
        %21.7) Compute Theorem 2   
        if ~isempty(c_H2)  
             if ~isempty(c_Hinf) 

                optimize(set_LMIs_partial,...
                    c_H2*rho+c_Hinf*gamma,SDP_settings); 

             else 

                 optimize(set_LMIs_partial,...
                     c_H2*rho,SDP_settings);

             end
        else 
            if ~isempty(c_Hinf) 

                optimize(set_LMIs_partial,...
                    c_Hinf*gamma,SDP_settings); 

            else 

                 optimize(set_LMIs_partial,[],SDP_settings);

            end 
        end

        %21.8) Extract decision variable by Theorem 7

        optimal_Tilde_W = value(Tilde_W);
        optimal_Tilde_X = value(Tilde_X);

        %21.9) Update Feedback matrix

        KD_partial=optimal_Tilde_W*inv(optimal_Tilde_X);
        Kpf_partial = Kpf_partial + KD_partial * (Q_p * L_p') / 2;

        
    % 21.10) End Optimizaition Process
    
    partial_optimization_time = toc;
    
%% 22) State-space representation of the closed-loop system designed via Theorem 7.

    % 22.1) State-Space for Gcdy(s) 
    
    Gcdys_partial=...
        ss(A_x+B_u*Kpf_partial,B_d,C_y+D_y*Kpf_partial,E_y);

    % 22.2) State-Space for Gcdz(s) 
    
    Gcdzs_partial=...
        ss(A_x+B_u*Kpf_partial,B_d,C_z+D_z*Kpf_partial,E_z); 

%% 23) Compute H_2 and H_inf norms of closed-loop system designed via Theorem 7.

    %23.1) H_2 norm of Gcdy(s)
    
    H2_norm_Gcdys_partial=norm(Gcdys_partial,2);
    
    %23.2) H_inf norm of Gcdz(s)
    
    Hinf_norm_Gcdzs_partial=norm(Gcdzs_partial,'inf');
    
%% 24) Compute poles of closed-loop system designed via Theorem 7.

    Poles_Gcd_Partial=cplxpair(pole(Gcdys_partial));
    
%% 25) Obtain Damping Coefficients and Natural Frequency.

[wn_theo7,zeta_theo7,p_theo7]=damp(Gcdys_partial);
cont=1;
    
%% 26) Compute Gcdy(s) response of unit impulse response

    Compute_yt_ut_impulse_response_partial_method
    
%% 27) Make Tables

    Compute_Tables
          
%% 28) Bode plots for open-loop and closed-loop LCTI-MIMO systems.

    figure 
    subplot(133)
    sigmaplot(Gcdzs_classic,Gcdzs_partial,Canon_Godzs)
    title('')
    legend('Gcdz(s) Classic','Gcdz(s) Partial','Godz(s)','FontSize',10)
    grid on;
    set(findall(gcf, 'Type', 'line'), 'LineWidth', 3); % Set thickness for all lines

    subplot(132)
    sigmaplot(Gcdzs_classic,Gcdzs_partial)
    title('')
    grid on;
    set(findall(gcf, 'Type', 'line'), 'LineWidth', 3); % Set thickness for all lines
    
    subplot(131)
    sigmaplot(Gcdzs_classic)
    title('')
    grid on;
    set(findall(gcf, 'Type', 'line'), 'LineWidth', 3); % Set thickness for all lines

    sgtitle('Singular Values G_d_z(s)')

%% 29) transient response plots of g(t)^2 to a unit impulse disturbance for the open-loop and closed-loop LCTI-MIMO system.

    figure
    subplot(133)
    plot(t_Gcdys_impulse_classic,y2_Gcdys_impulse_classic,...
         t_Gcdys_impulse_partial,y2_Gcdys_impulse_partial,...
         t_Canon_Godys_impulse,y2_Canon_Godys_impulse);
    xlabel('t')
    ylabel('y^2(t)')
    legend('Gcdy(s) classic','Gcdy(s) partial','Gody(s)')
    grid on;
    set(findall(gcf, 'Type', 'line'), 'LineWidth', 3); 
    
    subplot(132)
    plot(t_Gcdys_impulse_classic,y2_Gcdys_impulse_classic,...
         t_Gcdys_impulse_partial,y2_Gcdys_impulse_partial);
    xlabel('t')
    ylabel('y^2(t)')
    grid on;
    xlim([0,7])
    set(findall(gcf, 'Type', 'line'), 'LineWidth', 3); 
    
    subplot(131)
    plot(t_Gcdys_impulse_classic,y2_Gcdys_impulse_classic);
    xlabel('t')
    ylabel('y^2(t)')
    xlim([0,2.5])
    grid on;
    set(findall(gcf, 'Type', 'line'), 'LineWidth', 3); 
    
    sgtitle('Impulse Response y(t)')
    
%% 30) Plot of u(t) for the open-loop and closed-loop LCTI-MIMO system in response to a unit step disturbance.    
    
    figure
    subplot(121)
    plot(t_Gcdys_impulse_classic,u2_Gcdys_impulse_classic(:,1),...
         t_Gcdys_impulse_partial,u2_Gcdys_impulse_partial(:,1));
    xlim([0 2.5])
    title('u^2_1(t)')
    xlabel('t')
    ylabel('u^2_1(t)')
    legend('G_c_d_y(s) classic','G_c_d_y(s) parcial')
    grid on;
    set(findall(gcf, 'Type', 'line'), 'LineWidth', 3); 
    
    subplot(122)
    plot(t_Gcdys_impulse_classic,u2_Gcdys_impulse_classic(:,2),...
         t_Gcdys_impulse_partial,u2_Gcdys_impulse_partial(:,2));
    xlim([0 2.5])
    title('u^2_2(t)')
    xlabel('t')
    ylabel('u_2(t)')
    grid on;
    set(findall(gcf, 'Type', 'line'), 'LineWidth', 3); 
    
    sgtitle('Impulse response u(t)')

%% 31) Plots of the impulse response of partial fractions and transfer functions.

    figure
    subplot(131)
    impulseplot(GAs,'-k',GNAs,'-r',Godys,'-b',Godzs,'-y');
    title('')
    legend('GA(s)','GNA(s)','Gody(s)','Godz(s)','FontSize',10)
    grid on;
    set(findall(gcf, 'Type', 'line'), 'LineWidth', 3);
    
    subplot(132)
    impulseplot(GAs,'-k',GNAs,'-r',Godys,'-b',Godzs,'-y');
    title('')
    grid on;
    xlim([0 75])
    set(findall(gcf, 'Type', 'line'), 'LineWidth', 3);
    
    subplot(133)
    impulseplot(GAs,'-k',GNAs,'-r',Godys,'-b');
    title('')
    grid on;
    xlim([0 75])
    set(findall(gcf, 'Type', 'line'), 'LineWidth', 3);
    
    sgtitle('Resposta ao impulso')
    
    figure
    sigma(GAs,'-k',GNAs,'-r',Godys,'-b',Godzs,'-y');
    title('Singular Values','FontSize',12)
    grid on;
    legend('GA(s)','GNA(s)','Gody(s)','Godz(s)','FontSize',10)
    set(findall(gcf, 'Type', 'line'), 'LineWidth', 3);
    
%% 32) Pole Location Plot

    %32.1)  Eigenvalues (Poles) to be plotted.
    All_poles=[ Open_Loop_Poles',...
                  Poles_Gcd_classic',...  
                  Poles_Gcd_Partial'];

    %32.2) Computing region boundary curves.      
    [  x_horizontal_strip,y_lower_horizontal_strip,...
        y_upper_horizontal_strip,...
        x_sector,y_lower_sector,y_upper_sector,...
        x_disk,y_disk,...
        x_alphav,y_alphav,...
        x_betav,y_betav ] = coordinates_D_regions(...
        All_poles,alpha_v,beta_v,theta_s,r_d,q_d,w_H,e_P); 

    %32.3) Graph            
    figure
    subplot(121)
    plot(real(Open_Loop_Poles),imag(Open_Loop_Poles),'ok',...
         real(Poles_Gcd_classic),...
         imag(Poles_Gcd_classic),'+r',...
         real(Poles_Gcd_Partial),...
         imag(Poles_Gcd_Partial),'xb',...
         x_alphav,y_alphav,'-k',...
         x_betav,y_betav,'-k',...
         x_sector,y_lower_sector,'-k',...
         x_sector,y_upper_sector,'-k',...
         x_disk,y_disk,'-k',...
         x_horizontal_strip,y_lower_horizontal_strip,'-k',...
         x_horizontal_strip,y_upper_horizontal_strip,'-k',...
         'LineWidth',2,'MarkerSize',8);
   legend('Open','Classic','Partial','Boundary','FontSize',10) 
   grid on
   
   subplot(122)
   plot(real(Open_Loop_Poles),imag(Open_Loop_Poles),'ok',...
         real(Poles_Gcd_classic),...
         imag(Poles_Gcd_classic),'+r',...
         real(Poles_Gcd_Partial),...
         imag(Poles_Gcd_Partial),'xb',...
         x_alphav,y_alphav,'-k',...
         x_betav,y_betav,'-k',...
         x_sector,y_lower_sector,'-k',...
         x_sector,y_upper_sector,'-k',...
         x_disk,y_disk,'-k',...
         x_horizontal_strip,y_lower_horizontal_strip,'-k',...
         x_horizontal_strip,y_upper_horizontal_strip,'-k',...
         'LineWidth',2,'MarkerSize',8);
   ylim([-2. 2])
   xlim([-2.9 0])
   grid on
   sgtitle('Poles and Location in the Complex Plane','FontSize',12)
