    ...
    %1) Damping coefficient, Natural Frequency, and Gain of the partial functions
        ...
        zeta_c(1,1) = exp(-2+1/30);  % Function 1
        zeta_c(1,2) = exp(-2+2/30); % Function 2
        zeta_c(1,3) = exp(-2+3/30); % Function 3
        zeta_c(1,4) = exp(-2+4/30);  % Function 4
        zeta_c(1,5) = exp(-2+5/30);  % Function 5

        zeta_c(1,6) = 0.2;  % Function 6    

        %1.2) Input Natural frequency
        w_c(1,1) = 0.9^(1/30);  % Function 1
        w_c(1,2) = 0.9^(2/30); % Function 2
        w_c(1,3) = 0.9^(3/30); % Function 3
        w_c(1,4) = 0.9^(4/30);  % Function 4
        w_c(1,5) = 0.9^(5/30);  % Function 5

        w_c(1,6) = 0.1;  % Function 6    

        %1.3) Gain
        KYc(1:6)=0.15*ones(1,6);
        KZc(1:6)=0.15*ones(1,6);
    ...
    %2) Make G_c(s), G_ody(s) and G_odz(s)  
        ...
        GAs=tf(0,1); %Make G_A(s)
        GNAs=tf(0,1); %Make G_NA(s)

        for c=1:6 %loop for add G_c(s) in G_ody(s) and G_odz(s)
    ...
    tic
        ...
        if number_of_non_controlable_states == 0
            ...
            % 15.10.3) Make LMIs for theorem 7
            set_LMIs_theorem_7 = Tilde_X>=eps*eye(n_ola);

            % 15.10.4) Make LMI for Re(s)<=-alpha_v
            if ~isempty(alpha_v)
                Tilde_Lambda_Valpha = Tilde_Lambda_p+alpha_v*eye(n_ola);
            ...
            if ~isempty(beta_v)
                Tilde_Lambda_Vbeta = -Tilde_Lambda_p-beta_v*eye(n_ola);
                set_LMIs_theorem_7 = [ set_LMIs_theorem_7,...
            ...
            if ~isempty(r_d)
                Tilde_Lambda_Dqr = Tilde_Lambda_p+q_d*eye(n_ola); 
                set_LMIs_theorem_7 = [ set_LMIs_theorem_7,...
                    [ -r_d*Tilde_X,...
    ...
    percentual_discrepance_optimization_time=...
        100*(time_theorem_7-time_theorem_2)/time_theorem_2;
